...
float calculate_distance_sum(VI &individual) {
    float D = 0.0;
...
int find_index(VI &individual, int cell) {
    for (int i = 0; i < cell_num; i++) {
...
VI heuristic_crossover(VI &parent1, VI &parent2) {
    VI offspring(cell_num);
    bool visited[cell_num];
    memset(visited, 0, cell_num * sizeof(bool));
    int start_cell = rand() % cell_num;
    ...
    for (int i = 1; i < cell_num; i++) {
        ...
        for (auto &candidate_cell : candidate_cells) {
...
void swap_mutation(VI &individual) {
    if (rand() % 100 <= (swap_mutation_probability * 100.0)) {
        int index1 = rand() % cell_num, index2 = rand() % cell_num;
...
void best_choice_combination(VI &individual) {
    ...
    float distance_sum[cell_num * 4];
    int from[cell_num * 4];
    for (int starting_choice = 0; starting_choice < 4; starting_choice++) {
        fill(distance_sum, distance_sum + cell_num * 4, DISTANCE_MAX);
...
void evolve_next_generation() {
    ...
    for (int i = 0; i < population_size; i++) {
        if (rand() % 100 <= (crossover_probability * 100.0)) {
            VI &parent1 = population[roulette_wheel_selection()];
            VI &parent2 = population[roulette_wheel_selection()];
...
int main(int argc, char *argv[])
{
    ...
    string output_file = test_case + "/GA.txt";
    ...
    for (int i = 0; i < cell_num; i++) {
        float min_intra_path_length = min(intra_path_length[i * 4 + 0], intra_path_length[i * 4 + 1]);
        for (int j = 0; j < 4; j++) {
    ...
    for (int generation = 1; generation <= max_generation; generation++) {
        ...
        cout << "Best distance sum = " << best_distance_sum << '\n';
        cout << '\n';
        */
    }
    
    cout << "GA : " << best_distance_sum << '\n';
